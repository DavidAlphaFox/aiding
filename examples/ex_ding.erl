...
-export([corp_send_to_user/2,corp_send_to_depart/2]).
access_token()->
...
corp_send_to_user(UserList,Message)->
    AgentID = ai_ding_conf:app_id(undefined),
    AccessToken = access_token(),
    ai_ding_message:corp_send_to_users(AgentID,AccessToken,#ai_ding_message{
                                                              type = text,
                                                              content = ai_string:to_string(Message)
                                                             },UserList).
corp_send_to_depart(DepartList,Message)->
    AgentID = ai_ding_conf:app_id(undefined),
    AccessToken = access_token(),
